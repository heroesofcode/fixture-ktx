...
object Fixture {
    ...
    private fun generateValue(type: KType, propertyName: String): Any {
        ...
        return when {
            ...
            classifier == Set::class -> {
                val argumentType = type.arguments.first().type
                    ?: throw IllegalArgumentException("Unsupported set type: $type")
                generateSet(argumentType, Random.nextInt(RANGE_INT_INIT, RANGE_INIT_END))
            }
            classifier.java.isArray -> {
                val elementType = type.arguments.first().type
                    ?: throw IllegalArgumentException("Unsupported array type: $type")
                generateArray(elementType, Random.nextInt(RANGE_INT_INIT, RANGE_INIT_END))
            }
    ...
    /**
     * Generates a set of random values for the specified element type [elementType].
     *
     * This function creates a set of the specified [elementType] and [size]. It then fills
     * the set with random values generated by the `generateValue` function, ensuring all elements
     * are unique.
     *
     * @param elementType the KType of the set elements.
     * @param size the number of elements to generate.
     * @return a set of random values of the specified element type.
     * @throws IllegalArgumentException if the set type is unsupported.
     */
    private fun generateSet(elementType: KType, size: Int): Set<Any> {
        val set = mutableSetOf<Any>()
        while (set.size < size) {
            set.add(generateValue(elementType, ""))
        }
        return set
    }

    /**
     * Generates an array of random values for the specified element type [elementType].
     *
     * This function creates an array of the specified [elementType] and [size]. It then fills
     * the array with random values generated by the `generateValue` function.
     *
     * @param elementType the KType of the array elements.
     * @param size the number of elements to generate.
     * @return an array of random values of the specified element type.
     * @throws IllegalArgumentException if the array type is unsupported.
     */
    private fun generateArray(elementType: KType, size: Int): Any {
        val elementClass = elementType.jvmErasure.java
        val array = java.lang.reflect.Array.newInstance(elementClass, size)
        for (i in 0 until size) {
            java.lang.reflect.Array.set(array, i, generateValue(elementType, ""))
        }
        return array
    }
